...
def data_loader(cfg:DictConfig) -> Union[Dataset, DatasetDict]:
  ...
  if cfg.source_type.lower() == "hf":
    dataset_path = cfg.hf_path
    dataset = load_ner_dataset(dataset_path, source_type=cfg.source_type.lower()) 
  elif cfg.source_type.lower() == "local":
    dataset_path = to_absolute_path(cfg.data_folder)
    dataset = load_ner_dataset(dataset_path, source_type=cfg.source_type.lower(), file_type=cfg.file_type)
