...
def build_base_training_components(cfg:DictConfig, output_dir:str, device:str, use_wandb, wandb_run=None, run_artifact=None):
    ...
    tokenized_train = train_dataset.map(tokenize_fn, batched=True, remove_columns=train_dataset.column_names, load_from_cache_file=False, keep_in_memory=True)
    tokenized_val = val_dataset.map(tokenize_fn, batched=True, remove_columns=val_dataset.column_names, load_from_cache_file=False, keep_in_memory=True)
    logger.info(f"Tokenized Train len: {len(tokenized_train)}")
    logger.info(f"Tokenized Val len: {len(tokenized_val)}")
