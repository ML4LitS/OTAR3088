...
def fetch_terms_per_page(session, url, params):
...
# Function to split large lists into smaller chunks
def split_into_chunks(lst, chunk_size):
    """Yield successive chunk_size-sized chunks from a list."""
    for i in range(0, len(lst), chunk_size):
        yield lst[i:i + chunk_size]

# Construct search queries with a maximum of MAX_TERMS_PER_QUERY terms per query
def construct_search_queries(df, max_terms_per_query=MAX_TERMS_PER_QUERY):
